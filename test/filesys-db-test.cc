...
DECLARE_TEST(DBInit) {
    DurableStoreImpl* impl = new FileSystemStore();
...
DECLARE_TEST(SingleTypeIterator) {
    ...
    while(iter->next() == 0) {
        Builder b;
        IntShim key(b); // XXX/bowei -- can't use auto seems like g++ bug
        
        CHECK(iter->get(&key) == 0);
        
        log_notice("/test", "key = %d", key.value());
...
DECLARE_TEST(SingleTypeCache) {
    ...
    log_info("/test", "flamebox-ignore ign1 cache already at capacity");
    ...
    log_info("/test", "flamebox-ignore-cancel ign1");
