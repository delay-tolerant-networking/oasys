...
/*!
 * Same as above but implemented with a Hash table. [bowei] I've tried
 * this out in several cases replacing the original cache, and it
 * turned out that the hash table implementation (versus red-black
 * tree for a map) was slower! It's not always a clear performance
 * win.
 */
template<typename _Key, typename _Val, typename _CacheHelper>
class HashCache : Logger {
public:
    /*!
     * LRU list entry.
     */
     ...
     */
    typedef LRUList<LRUListEnt> CacheList;
    typedef __gnu_cxx::hash_map<_Key, typename CacheList::iterator, 
                                Hasher> CacheMap;
    ...
    private:
        ...
        Handle(HashCache* cache, typename CacheList::iterator itr)
            : cache_(cache), itr_(itr) {}
    };
    ...
    bool get(const _Key& key, _Val* valp = 0, bool pin = false,
             Handle* handle = 0)
    {
        ScopeLock l(&lock_, "HashCache::get_and_pin");
    ...
    int pin(const _Key& key) 
    {
        ScopeLock l(&lock_, "HashCache::pin");

        typename CacheMap::iterator i = cache_map_.find(key);
        ASSERT(i != cache_map_.end());
        
        ScopeLock ll(&i->second->lock_, "HashCache::pin");
        int count = ++i->second->pin_count_;
        log_debug("pin(%s): pinned entry pin_count=%d size=%zu",
                  InlineFormatter<_Key>().format(key),
                  count,
    ...
    int pin(Handle handle)
    {
        ScopeLock l(&handle.itr_->lock_, "HashCache::pin");
        int count = ++handle.itr_->pin_count_;
        log_debug("pin(%s): pinned entry pin_count=%d size=%zu",
                  InlineFormatter<_Key>().format(handle.itr_->key_),
                  count,
    ...
    int unpin(const _Key& key) 
    {
        ScopeLock l(&lock_, "HashCache::unpin");
    ...
    int unpin(Handle handle)
    {
        ScopeLock l(&handle.itr_->lock_, "HashCache::unpin");
    ...
    bool put_and_pin(const _Key& key, const _Val& val,
                     Handle* handle = 0) 
    {
        ScopeLock l(&lock_, "HashCache::put_and_pin");
    ...
    void evict(const _Key& key) 
    {
        ScopeLock l(&lock_, "HashCache::evict");
    ...
    class ScopedUnpin {
    public:
        ScopedUnpin(HashCache* cache, const _Key& key)
            : cache_(cache), key_(key) {}
